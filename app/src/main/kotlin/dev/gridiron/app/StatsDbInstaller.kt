...
internal class StatsDbInstaller(private val context: Context) {
    ...
    /**
     * Downloads the latest database from the repository's rolling `data`
     * release over the installed one. Returns a message for the user; the
     * caller restarts the app so every screen reopens on the new file.
     */
    suspend fun refresh(): Result<String> = withContext(Dispatchers.IO) {
        runCatching {
            val manifest = get("$RELEASE/manifest.json").use { it.readBytes().decodeToString() }
            val schema = Regex("\"schema_version\"\\s*:\\s*(\\d+)").find(manifest)?.groupValues?.get(1)?.toInt()
            check(schema == SCHEMA_VERSION) { "Published data is schema $schema; this app needs $SCHEMA_VERSION. Update the app." }
            val dest = File(context.noBackupFilesDir, DB_NAME)
            val tmp = File(dest.path + ".tmp")
            GZIPInputStream(get("$RELEASE/stats.db.gz")).use { input -> tmp.outputStream().use { input.copyTo(it, BUFFER) } }
            check(tmp.renameTo(dest)) { "couldn't move the stats database into place" }
            val built = Regex("\"built_at\"\\s*:\\s*\"([^\"]+)\"").find(manifest)?.groupValues?.get(1)
            "Stats updated" + (built?.let { " (built $it)" } ?: "")
        }
    }
    ...
    private fun get(url: String) = (URL(url).openConnection() as HttpURLConnection).run {
        ...
        readTimeout = 60_000
        check(responseCode == 200) { "Download failed: HTTP $responseCode" }
        inputStream
    }

    private companion object {
        const val RELEASE = "https://github.com/Palm9999/Lame/releases/download/data"
